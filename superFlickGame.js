...
function updateBall(kin, ball, bin, bin2, bin3, dragDrop)
{
	...
	var collisionDamper = 1.0; // 100% energy loss
	var floorFriction = 5; 
	...
	function resetBallPos(ball)
	{
		...
		ball.vy *= 0;
		ball.vx *= 0;
		ball.vx *= (0 - collisionDamper);
		ball.vy *= (0 - collisionDamper);	
	}
}
...
function imageGet()
{
	...
	xmlhttp.open("GET" , "http://daisy.colum.edu/SuperFlick/index.php?jobID=2", true);
	xmlhttp.send();
	document.write('xmlhttp.responseText');*/

	var imgRequest = $.ajax({
		url: "http://daisy.colum.edu/SuperFlick/downloader.php$callback=?",
		async: true,
		cache: true,
		crossDomain: true,
		data: "jobID=2",
